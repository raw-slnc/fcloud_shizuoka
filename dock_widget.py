...
class _TableInputFilter(QObject):
    ...
    def eventFilter(self, obj, event):
        et = event.type()
        if et == QEvent.Wheel and (event.modifiers() & Qt.ShiftModifier):
            sb = self._table.horizontalScrollBar()
            sb.setValue(sb.value() - event.angleDelta().y())
            return True
        if et == QEvent.KeyPress and event.matches(QKeySequence.Copy):
...
class FcloudDockWidget(QDockWidget):
    """UI 実体（FcloudWindow）を収める薄い Dock コンテナ。
    フロート時の window-flag 調整と Alt+Tab 用の HWND オーナー解除だけを受け持つ
    （GPKG Editor と同じ分離）。"""
    ...
    def _on_floating_state_changed(self, is_floating):
        widget = self.widget()
        if widget is not None and hasattr(widget, '_on_floating_state_changed'):
    ...
    def toggle_floating_fullscreen(self, checked):
        """フロート中のみ有効。ウィンドウ側の「全画面表示」チェックから委譲される。"""
        if not self.isFloating():
            return False
        if checked:
            self._pre_fullscreen_geometry = self.geometry()
            self.showMaximized()
            # showMaximized() がネイティブウィンドウのオーナー関係を再設定するため解除し直す
    ...
    def closeEvent(self, event):
        # ドック純正の ✕ でのみ発火（ツールバートグルの setVisible(False) は経由しない）
        self.closed.emit()
...
class FcloudWindow(HoanrinMixin, MoriMixin, KeikakuMixin, RinchiMixin, ShinrinboMixin, QWidget):
    ...
    # ------------------------------------------------------------------
    # フロート状態と「全画面表示」チェックの連携（実処理は Dock 側へ委譲）
    # ------------------------------------------------------------------

    def attach_dock_widget(self, dock_widget):
        self._dock_widget = dock_widget
        self.chk_fullscreen.setEnabled(dock_widget.isFloating())

    def _on_floating_state_changed(self, is_floating):
        """FcloudDockWidget.topLevelChanged から呼ばれる。"""
        self.chk_fullscreen.setEnabled(is_floating)
        if not is_floating and self.chk_fullscreen.isChecked():
            self.chk_fullscreen.blockSignals(True)
            self.chk_fullscreen.setChecked(False)
            self.chk_fullscreen.blockSignals(False)
    ...
    def _build_ui(self):
        ...
        self.chk_fullscreen = QCheckBox('全画面表示')
        self.chk_fullscreen.setToolTip('フロート中のみ有効。ウィンドウを最大化表示する')
        self.chk_fullscreen.setEnabled(False)
        self.chk_fullscreen.toggled.connect(self._on_fullscreen_toggled)
        _corner_w = QWidget()
        _corner_l = QHBoxLayout(_corner_w)
        _corner_l.setContentsMargins(0, 3, 0, 3)
        _corner_l.addWidget(self.chk_fullscreen)
    ...
    def _teardown_visible_state(self):
        ...
        プラグイン生成レイヤー除去・保留リクエスト abort）。FcloudDockWidget.closed から
        プラグイン経由で呼ばれる。"""
        self._restore_selection_color()
